...
def test_standby_setpoint_preconditions(lessons, state):
    ...
    with VerdiController(sim, model=Model.V5, allow_writes=state != "writes_disabled") as c:
        if state == "on":
            ...
            sim.set_faults(2)
            c.stop()  # Standby does not clear the active fixture fault.
        before = writes(sim)
...
@pytest.mark.parametrize("name", LESSONS)
def test_self_contained_notebooks_execute(name, hardware_path, tmp_path, monkeypatch):
    ...
    functions = {node.name for node in notebook_tree.body if isinstance(node, ast.FunctionDef)}
    assert {f"simulate_{name}", "validate_power", "hardware_connection"} <= functions
    ...
    expected = {
        "read_status": "Expected PermissionError",
